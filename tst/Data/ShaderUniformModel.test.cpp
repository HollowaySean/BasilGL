...
TEST_CASE("Data_ShaderUniformModel_addUniform") {
    ...
    unsigned int boolID = dataModel.addUniform(std::make_shared<GLUniformScalar<bool>>(true, "myBool"));
    // From scalar value
    unsigned int floatID = dataModel.addUniform(1.5f, "myFloat");
    // From pointer
    int intValue = -15;
    unsigned int intID = dataModel.addUniform(&intValue, "myInt");
    // From std::vector
    unsigned int uintID = dataModel.addUniform(std::vector<unsigned int>({ 2, 4 }), "myUint");

    // Overwrite case
    unsigned int floatIDOverwrite = dataModel.addUniform(2.5f, "myFloat");

    SECTION("Saves uniforms to container by name") {
        CHECK(dataModel.getUniforms().size() == 4);

        CHECK(*(reinterpret_cast<bool*>(dataModel.getUniform("myBool").value()->getData()))
            == true);
        CHECK(*(reinterpret_cast<float*>(dataModel.getUniform("myFloat").value()->getData()))
            == 2.5f);
        CHECK(*(reinterpret_cast<int*>(dataModel.getUniform("myInt").value()->getData()))
            == intValue);
        CHECK((reinterpret_cast<unsigned int*>(dataModel.getUniform("myUint").value()->getData()))[0]
            == 2);
        CHECK((reinterpret_cast<unsigned int*>(dataModel.getUniform("myUint").value()->getData()))[1]
            == 4);
    }

    SECTION("Saves uniforms to container with ID") {
        CHECK(*(reinterpret_cast<bool*>(dataModel.getUniform(boolID).value()->getData()))
            == true);
        CHECK(*(reinterpret_cast<float*>(dataModel.getUniform(floatID).value()->getData()))
            == 2.5f);
        CHECK(*(reinterpret_cast<int*>(dataModel.getUniform(intID).value()->getData()))
            == intValue);
        CHECK((reinterpret_cast<unsigned int*>(dataModel.getUniform(uintID).value()->getData()))[0]
            == 2);
        CHECK((reinterpret_cast<unsigned int*>(dataModel.getUniform(uintID).value()->getData()))[1]
            == 4);
    }
    ...
    SECTION("Saves uniform containing texture location") { BASIL_LOCK_TEST
        ...
        CHECK(*(reinterpret_cast<int*>(dataModel.getUniform(texID).value()->getData()))
...
TEST_CASE("Data_ShaderUniformModel_setUniformValue") {
    ...
    SECTION("Set to GLUniform object") {
        CHECK(dataModel.setUniform(std::make_shared<GLUniformScalar<int>>(5, "myInt"), ID));
        CHECK_FALSE(dataModel.setUniform(std::make_shared<GLUniformScalar<int>>(5, "myInt"), -1));

        CHECK(*(reinterpret_cast<int*>(dataModel.getUniform(ID).value()->getData()))
            == 5);
    }
    ...
    SECTION("Set to vector") {
        ...
        CHECK((reinterpret_cast<int*>(dataModel.getUniform(ID).value()->getData()))[0]
            == 5);
        CHECK((reinterpret_cast<int*>(dataModel.getUniform(ID).value()->getData()))[1]
            == 7);
    }
...
TEST_CASE("Data_ShaderUniformModel_Builder") {
    SECTION("Builds model correctly") {
        ...
        auto model = ShaderUniformModel::Builder()
            .withUniform(std::make_shared<GLUniformScalar<float>>(1.5f, "myFloat"))
